...
def plot_spherical_profiles(planet=None,useU=False,Till=False,planet1loc=None, planet2loc=None, loadfromfile=False):    
    ...
    if not loadfromfile:
        ...
        ax[1].scatter(planet.A1_r / R_earth, np.log10(1e-9*planet.A1_P),c=colour,s=0.1)
    ...
    fig.savefig('/user/home/qb20321/hammer/paper2_image/profile_m1d0.png',dpi=600, format="png", transparent=True,bbox_inches='tight')
    # plt.show()
...
def sw_plot(loc, snapshot_id=None,ax=None, npt=1e9, ax_lim=3.0,offcenter=None, \
            outtime=None, midplane=False,rhocontour=False, quarter=False, \
            belowZ=True, figsz=10,plotxy=True,selpid=None,EOS=None,mantleplot=True,\
            plotwhole=False,pureplot=False,psize=5,PADX=10,PADY=10,tcx='w',tcy='w',
            SPH=True,load_region=None):
...
#             data = sw.load(loc)
    ...
    if midplane:
        sel    = np.where(np.logical_and(pos[:, 2] < 0.05 * R_earth, pos[:, 2] > -0.05 * R_earth))[0]
        mat_id = mat_id[sel]
        m      = m[sel]
        id     = id[sel]
        pos    = pos[sel]
    ...
    #Restrict to z < 0
        ...
        if plotxy:
            sel    = np.where(np.logical_and(pos[:, 2] < 0.1 * R_earth, pos[:, 2] > -600 * R_earth))[0]
        else:
            sel    = np.where(np.logical_and(pos[:, 1] < 0.1 * R_earth, pos[:, 1] > -600 * R_earth))[0]
        mat_id = mat_id[sel]
        id     = id[sel]
        m      = m[sel]
        pos    = pos[sel]
    ...
    if EOS=='iron':
        core_sel   = np.logical_or(mat_id==Di_mat_id["ANEOS_iron_2"],mat_id==Di_mat_id["ANEOS_iron"])
        mantel_sel = np.logical_or(mat_id==Di_mat_id["ANEOS_forsterite_2"],mat_id==Di_mat_id["ANEOS_forsterite"])
    elif EOS=='alloy':
        core_sel   = np.logical_or(mat_id==Di_mat_id["ANEOS_alloy_2"],mat_id==Di_mat_id["ANEOS_alloy"])
        mantel_sel = np.logical_or(mat_id==Di_mat_id["ANEOS_forsterite_2"],mat_id==Di_mat_id["ANEOS_forsterite"])
    elif EOS=='Till':
        core_sel   = np.logical_or(mat_id==Di_mat_id["Til_iron_2"],mat_id==Di_mat_id["Til_iron"])
        mantle_sel = np.logical_or(mat_id==Di_mat_id["Til_granite_2"],mat_id==Di_mat_id["Til_granite"])
    else:
...
#    colour/=255.0
    ...
    if plotwhole:
        ...
        if selpid is not None:
                ax.scatter(pos[np.in1d(id,selpid),0],pos[np.in1d(id,selpid),1],s=1,c='lime')
    else:
        if plotxy:
            if mantleplot:
                ax.scatter(
            ...
            if selpid is not None:
                axx=ax.scatter(pos[np.in1d(id,selpid),0],pos[np.in1d(id,selpid),2],s=1,c='lime')
...
#    ax.tick_params(axis="y",direction="in")
    ...
    if outtime is not None:
        ...
        ax.text(0.97,0.97,r'{0:6.2f} h'.format(outtime/3600),
    ...
    if pureplot:
        ...
        scalebar = ScaleBar(1, "m",fixed_value=0.5 * Bound.R_earth, 
                    scale_formatter=lambda value, unit: f"{value/Bound.R_earth}" + r"$R_\oplus$",
                   color='w',frameon=False,location="lower left",font_properties={"size":14})
        ...
        fig.savefig('/user/home/qb20321/hammer/denseplanets/figures/select_particle.png',
                    format='png',dpi=300,facecolor='k',bbox_inches='tight')
...
def print_impact_info(loc_tar,loc_imp, Xves=1.0, verbose=1):
    ...
    pos_tar, vel_tar, h_tar, m_tar, rho_tar, p_tar, u_tar, matid_tar, R_tar = loadsw_to_woma(loc_tar)
    pos_imp, vel_imp, h_imp, m_imp, rho_imp, p_imp, u_imp, matid_imp, R_imp = loadsw_to_woma(loc_imp)
    ...
    if verbose:
        ...
        print('Target mass   = %.5f, number of partiles %d, Radius=%.5f'%(M_t/M_earth,len(pos_tar),R_tar/R_earth))
        print('Impactor mass = %.5f, number of partiles %d, Radius=%.5f'%(M_i/M_earth,len(pos_imp),R_imp/R_earth))
