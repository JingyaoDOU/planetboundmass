...
class Bound:
    ...
    def re_distribute(self, bid, rem_bid, verbose=1):
        ...
        c_rem_com = (
            np.sum(
                self.pos[self.bound == bid] * self.m[self.bound == bid, np.newaxis],
                axis=0,
            )
            / c_rem_m
        )
        c_rem_vel = (
            np.sum(
                self.vel[self.bound == bid] * self.m[self.bound == bid, np.newaxis],
                axis=0,
            )
            / c_rem_m
        )
    ...
    def all_in_one(
        ...
        d = {
            ...
            "targ_ratio_lr": (1 + self.gamma) * self.mass_ratio[0],
            ...
            "targ_ratio_slr": (1 + self.gamma) * self.mass_ratio[1:2],
